...
if True:
    ...
    # 9 clusters, spawns from grid clust, total average
    kmeans_clustering(
        n_clusters=len(spawns_grid_k9),
        name="kmeans_speed_clusters_bottlenecks_sessions/9_grid_clust_spawn_average",
        spatial_weight=1,
        dynamic_weight=1,
        init_links=spawns_grid_k9,
        timeframe=None,
        session_min=s_min,
        session_max=s_max,
        qc=qc
    ...
    run_kmeans_graph(
        ...
        init_links=spawns_grid_k5,
        timeframe=timesteps,
        cluster_colors=cluster_colors[len(spawns_grid_k5)],
        session_min=s_min,
        session_max=s_max,
        qc=qc
    ) 
    
    # 9 clusters, spawns from grid clust
    run_kmeans_graph(
        n_clusters=len(spawns_grid_k9),
        name="kmeans_speed_clusters_bottlenecks_sessions/9_grid_clust_spawn",
        init_links=spawns_grid_k9,
        timeframe=timesteps,
        cluster_colors=cluster_colors[len(spawns_grid_k9)],
        session_min=s_min,
        session_max=s_max,
        qc=qc
    ) 
