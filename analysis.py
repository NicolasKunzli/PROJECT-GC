...
if __name__ == "__main__":
    ...
    # ─────────────────────────────────────────────────────────────────────────
    slow_idx  = set(np.argsort(mean_speed)[:20])
    first_red = True
    ...
    for s in range(speed.shape[0]):
        ax.plot(t_axis, np.nanmean(speed[s], axis=1),
    ...
    fig.suptitle(f"Data Reliability Analysis  (all {DL.num_sessions} sessions)",
    ...
    nan_frac_per_session = np.mean(np.isnan(speed), axis=1)   # (S, N)
    nan_frac_flat        = nan_frac_per_session.flatten()
    ...
    # Free-flow speed: 85th-percentile per segment across all sessions & time
    speed_flat = speed.reshape(-1, speed.shape[-1])       # (S*T, N)
    free_flow  = np.nanpercentile(speed_flat, 85, axis=0) # (N,)
    free_flow  = np.where(free_flow == 0, np.nan, free_flow)

    # Congestion index CI ∈ [0, 1]:  0 = free flow,  1 = fully stopped
    ci     = np.clip(1.0 - speed / free_flow[np.newaxis, np.newaxis, :], 0, 1)
    ci_all = np.nanmean(ci, axis=0)   # (T, N)

    slow_idx_list  = list(np.argsort(mean_speed)[:20])
    slow_section_ids = [DL.index_to_section_id.get(i, i) for i in slow_idx_list]

    ci_corridor    = ci_all[:, slow_idx_list]    # (T, 20)
    speed_corridor = speed_all[:, slow_idx_list] # (T, 20)
    tick_step      = max(1, T // 10)

    # Plot: space-time speed heatmap
    ...
    im = ax.imshow(
        ...
        cmap="RdYlGn", vmin=vmin_sp, vmax=vmax_sp,
        interpolation="nearest",
    )
    '''# shockwave contour at 0.5 × free-flow threshold
    ff_corridor = free_flow[slow_idx_list]
    threshold   = 0.5 * ff_corridor[:, np.newaxis]
    shock_mask  = (speed_corridor.T < threshold).astype(float)
    ...
    # Plot: network-level congestion share over time
    THRESH           = 0.4
